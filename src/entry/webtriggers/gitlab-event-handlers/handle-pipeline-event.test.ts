...
import { TEST_TOKEN, MOCK_CLOUD_ID, BASE_URL } from '../../../__tests__/fixtures/gitlab-data';
...
describe('Gitlab events', () => {
  ...
  it('ignores event if the branch is not default and non-default branch wasn`t  set', async () => {
    ...
    await handlePipelineEvent(eventWithIncorrectRef, BASE_URL, TEST_TOKEN, MOCK_CLOUD_ID);
  ...
  it('ingests build event for main branch', async () => {
    ...
    await handlePipelineEvent(event, BASE_URL, TEST_TOKEN, MOCK_CLOUD_ID);
  ...
  it('ingests build events from a non-default branch which was set via project variable', async () => {
    ...
    await handlePipelineEvent(nonDefaultBranchEvent, BASE_URL, TEST_TOKEN, MOCK_CLOUD_ID);
